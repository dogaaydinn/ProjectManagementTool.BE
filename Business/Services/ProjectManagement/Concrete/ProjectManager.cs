...
public class ProjectManager : IProjectService
{
    ...
    public async Task<ServiceCollectionResult<ProjectGetDto?>> GetAllByManagerIdAsync(Guid managerId,ProjectSortOptions? projectSortOptions)
    {
        var result = new ServiceCollectionResult<ProjectGetDto?>();
        ...
        try
        {
            ...
            projects = projectSortOptions switch
            {
                ProjectSortOptions.Name => projects.OrderBy(p => p.Name).ToList(),
                ProjectSortOptions.Priority => projects.OrderBy(p => p.Priority).ToList(),
                ProjectSortOptions.Status => projects.OrderBy(p => p.Status).ToList(),
                ProjectSortOptions.DueDate => projects.OrderBy(p => p.DueDate).ToList(),
                _ => projects
            };
   ...
   public async Task<ServiceCollectionResult<ProjectGetDto?>> GetProjectByPriorityAsync(int priority, ProjectSortOptions? projectSortOptions)
{
    var result = new ServiceCollectionResult<ProjectGetDto?>();
   ...
   public async Task<ServiceCollectionResult<ProjectGetDto?>> GetProjectByStatusAsync(int status, ProjectSortOptions? projectSortOptions)
{
    ...
    public async Task<ServiceObjectResult<ProjectGetDto?>> ChangeManagerOfExistingProject(ChangeManagerOfExistingProjectDto changeManagerOfExistingProjectDto)
    {
        var result = new ServiceObjectResult<ProjectGetDto?>();
        ...
        try
        {
            ...
            // Get the project
            var project = await _projectDal.GetAsync(p => p.Id == changeManagerOfExistingProjectDto.ProjectId && p.IsDeleted == false);
            ...
            project!.ManagerId = changeManagerOfExistingProjectDto.NewManagerId;
    ...
    public async Task<ServiceObjectResult<ProjectGetDto?>> AssignTeamToProject(AssignTeamToProjectDto assignTeamToProjectDto)
    {
        var result = new ServiceObjectResult<ProjectGetDto?>();
        ...
        try
        {
            ...
            // Get the project
            var project = await _projectDal.GetAsync(p => p.Id == assignTeamToProjectDto.ProjectId && p.IsDeleted == false);
            ...
            if (AuthHelper.GetRole()!.Equals(UserRoles.Admin))
                hasAccess = true;
            else if (project!.ManagerId == loggedInUserId)
                hasAccess = true;
            
            if (!hasAccess)
            {
                result.Fail(new ErrorMessage("PRJM-537954", "You do not have access to assign teams to this project"));
                return result;
            }
            
            var team = await _teamProjectDal.GetAsync(tp => tp.ProjectId == assignTeamToProjectDto.ProjectId);
            await _teamProjectDal.AddAsync(
                    new TeamProject
                    {
                        TeamId = team!.Id,
